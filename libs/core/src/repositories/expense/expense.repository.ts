...
export class ExpenseRepository {
  ...
  async createExpense(data: Expense): Promise<any> {
    try {
      const expenseInsert = await this.expenseRepository.manager.insert(Expense, data);
      return { data: expenseInsert.identifiers[0].id, msg: 'Gasto creado exitosamente!', code: 200 }
    } catch (e) {
